...
def preprocess_inputs(t, inputs, device, nums=6):
    ...
    if "drainage_features" in inputs:
        drainage_features = normalize_drainage_features(inputs["drainage_features"])
        input_channels.append(drainage_features)

    processed_inputs = torch.cat(input_channels, dim=1).to(dtype=torch.float)
...
def normalize_drainage_features(drainage_features):
    """Normalise static drainage rasters before channel concatenation.

    Expected channel order for the v1 drainage dataset:
    drain_inlet_mask, drain_outfall_mask, pipe_mask, pipe_diameter,
    pipe_slope, pipe_capacity, pipe_cover_depth, distance_to_outfall.
    Unknown extra channels are passed through with scale 1.
    """
    drainage_features = torch.nan_to_num(
        drainage_features.float(), nan=0.0, posinf=0.0, neginf=0.0
    )
    n_channels = drainage_features.shape[1]
    base_scales = torch.tensor(
        [1.0, 1.0, 1.0, 3.0, 0.10, 10.0, 10.0, 20000.0],
        dtype=drainage_features.dtype,
        device=drainage_features.device,
    )
    if n_channels <= len(base_scales):
        scales = base_scales[:n_channels]
    else:
        extra = torch.ones(
            n_channels - len(base_scales),
            dtype=drainage_features.dtype,
            device=drainage_features.device,
        )
        scales = torch.cat([base_scales, extra], dim=0)
    scales = scales.view(1, n_channels, 1, 1, 1)
    return torch.clamp(drainage_features / scales, min=0.0, max=1.0)
