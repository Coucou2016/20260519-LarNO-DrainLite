...
def _conf_get(section, name, default=None):
    try:
...
def _conf_bool(section, name, default=False):
    value = _conf_get(section, name, default)
...
def run_test(location, model, exp_root, expr_id, data_root, config, device):
    ...
    load_info = load_state_dict_with_optional_lifting_expand(
        trainer.model,
        checkpoint_path,
        map_location="cpu",
        expand_lifting=_conf_bool(config.finetune, "expand_lifting", False),
        old_data_channels=int(_conf_get(config.finetune, "old_data_channels", 13)),
    )
    print(
        "Checkpoint loaded. "
        f"loaded={len(load_info['loaded'])}, expanded={load_info['expanded']}, "
        f"skipped={load_info['skipped'][:5]}"
    )
    trainer.model.eval()
    ...
    test_dataset = Dynamic2DFlood(
        ...
        drainage_features=_conf_get(config.data, "drainage_features", None),
    )
